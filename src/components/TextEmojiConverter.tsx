...
import { Copy, Download, Check, Smartphone, MessageCircle, Youtube, Instagram, Laptop, Sparkles } from 'lucide-react';
...
interface TextEmojiConverterProps {
...
export const TextEmojiConverter: React.FC<TextEmojiConverterProps> = ({
    textResult,
    platform,
    onPlatformChange,
    deviceMode,
    ...
    const getFormattedText = () => {
        ...
        // Note: NBSP and alignment are now handled in the worker.
    ...
    const handleDownload = () => {
        ...
        saveAs(blob, `photo-to-emoji-${platform}.txt`);
    };
                    ...
                    {textResult ? (
                        ...
                        >
                            <div className={`p-6 rounded-3xl border-2 border-gray-100 dark:border-gray-900 bg-gray-50/50 dark:bg-gray-950/50 backdrop-blur-sm ${deviceMode === 'mobile' ? 'shadow-2xl' : ''}`}>
                                <pre className={`art-grid text-[10px] sm:text-[12px] leading-none mx-auto inline-block min-w-full text-black dark:text-white select-all`}>
                                    {getFormattedText()}
                                </pre>
                            </div>
                            {deviceMode === 'mobile' && (
                        ...
                        <div className="h-full flex flex-col items-center justify-center space-y-6 text-center">
                            <div className="w-20 h-20 bg-gray-50 dark:bg-gray-900 rounded-3xl flex items-center justify-center text-gray-200 dark:text-gray-800 animate-pulse">
                                <Sparkles size={40} />
                            </div>
                            <div className="space-y-2">
                                <h3 className="text-sm font-black uppercase tracking-widest text-gray-400">Awaiting Transformation</h3>
                                <p className="text-[10px] text-gray-500 font-bold max-w-[200px] leading-relaxed">
                                    Emoji text art is optimized for mobile messaging and social feeds.
                                </p>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="absolute bottom-6 right-6 flex items-center space-x-2 p-2 bg-white/90 dark:bg-gray-900/90 backdrop-blur-md rounded-2xl shadow-2xl border border-white/20">
                        <button
                            ...
                            className={`
                                ...
                                ${copied ? 'bg-green-500 text-white' : 'text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/10'}
                            `}
                        ...
                        <button
                            ...
                            className="p-3 text-purple-600 hover:bg-purple-50 dark:hover:bg-purple-900/10 rounded-xl transition-all"
                            title="Download .txt"
                        >
                            <Download size={18} />
                        </button>
            ...
            <div className="px-6 py-4 border border-gray-100 dark:border-gray-800 rounded-[1.5rem] opacity-50">
                <div className="flex items-center justify-center space-x-4">
                    <div className="flex items-center space-x-2">
                        ...
                        <span className="text-[8px] font-black uppercase tracking-widest">Device: {deviceMode}</span>
                    </div>
                    <div className="flex items-center space-x-2">
                        <div className="w-2 h-2 rounded-full bg-gray-300" />
                        <span className="text-[8px] font-black uppercase tracking-widest">Platform: {platform}</span>
                    </div>
                </div>
            </div>
        </div>
